            ...
            for v in v_list:
                ...
                if k == 4:
                    for edge in edges:#edge = 'VK'
                        ...
                        '''
                        edges_edge = return_edges(problem, edge)#edges_edge = ['VCM','VDM']
                        for edgeedge in edges_edge:#edgeedge = 'VCM'
                            repeated = False
                            for edge_done in list_done:
                                if edgeedge != edge_done:
                                    aux.append(edgeedge)#aux = ['VDM','VK','VCM']
                                else:#ignorar este, pois não queremos mandar repetidos...
                                    repeated = True
                                    break
                            if repeated:
                                continue
                            ...
                            sum_w = float(0)
                            for cw in comb:
                                sum_w += list_V[cw]#sum_w = 4.3 + 19.64 + 20.4 = 44.34
                            comb_tup = (comb, sum_w)#comb_tup = (['VDM','VK'],44.34)
                            list_comb.append(comb_tup)
                            print_comb(list_comb)
                            print("k =",k)
                            input()
                        '''
                    #list_done.append(edge)
